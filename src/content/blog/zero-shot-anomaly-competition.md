...
date: "2026-04-24"
description: "What actually moved the needle in the VAND4 zero shot industrial track, and the things we tried that didn't."
...
write ups tend to describe only the winning path, here's the fuller version,
including the branches that went nowhere.

## The constraint that shapes everything

Zero shot means no anomalous examples, not few, none. You're given normal
...
**Segmentation as a post processing prior.** Similarity maps are blobby and
don't respect object boundaries. Using SAM to snap high scoring regions to
...
- **Hand tuned thresholds per category.** Fit the validation set, generalised
  ...
  function.
- **Larger encoders, uncritically.** More parameters didn't reliably mean
...
were made and cleaned up, not in which backbone produced the features. That's
probably specific to the regime, with labelled anomalies I'd expect the
